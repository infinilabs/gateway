...
func (c *Console) makeLogFunc(level string) func(call goja.FunctionCall) goja.Value {
	return func(call goja.FunctionCall) goja.Value {
		if format, ok := goja.AssertFunction(c.util.Get("format")); ok {
			...
			default:
				panic(errors.Errorf("unhandled log level: %v", level))
			}
		} else {
...
func Require(runtime *goja.Runtime, module *goja.Object) {
	...
	o.Set("debug", c.makeLogFunc(log.DebugStr))
	o.Set("info", c.makeLogFunc(log.InfoStr))
	o.Set("log", c.makeLogFunc(log.InfoStr))
	o.Set("warn", c.makeLogFunc(log.WarnStr))
	o.Set("error", c.makeLogFunc(log.ErrorStr))
}
