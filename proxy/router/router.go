...
func (r *Router) Handler(ctx *fasthttp.RequestCtx) {
	...
	uriObj:=ctx.URI()
	path := string(uriObj.PathOriginal())
	method := string(ctx.Method())

	//TODO optimize router performance
	//try hashmap first, learning from history, limited size, fixed memory, LRU, build and replace current hashmap in background
